...
def sklearn_eval(X,y,split,model):
    X_train, X_test, y_train, y_test = train_test_split(X,y,test_size=split,random_state=69)
...
def eval(X,y,dataset,model):
    for split in splits:
        preprocessy_test_size, preprocessy_preds, preprocessy_y_test = preprocessy_eval(X,y,split,model)
        sklearn_test_size, sklearn_preds, sklearn_y_test = sklearn_eval(X,y,split,model)
        preprocessy_accuracy = classification_report(preprocessy_y_test,preprocessy_preds,output_dict=True)['accuracy']
        sklearn_accuracy = classification_report(sklearn_y_test,sklearn_preds,output_dict=True)['accuracy']
        print(f'Dataset - {dataset}')
        print(f'Preprocessy test_size - {preprocessy_test_size} accuracy - {preprocessy_accuracy}')
