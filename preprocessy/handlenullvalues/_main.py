...
class NullValuesHandler:
    ...
    def __validate_input(self):
        if self.df is None:
            raise ValueError(f"Feature dataframe should not be of None type")

        if type(self.df) is not pd.core.frame.DataFrame:
            raise TypeError(f"Feature dataframe is not a valid dataframe.\nExpected object type: pandas.core.frame.DataFrame")

        if self.drop is None and self.fill_missing is None and self.fill_values is None:
            raise ArgumentsError(f"Expected one argument apart from dataframe, received none")

        if self.drop is not None:
            if self.fill_missing is not None or self.fill_values is not None:
                raise ArgumentsError(f"Received more than one arguments, expected atmost one argument")
            elif type(self.drop) is not bool:
                raise TypeError(f"Expected boolean value for argument \"drop\" ")
            elif self.drop==False:
                warnings.warn("Drop is False, thus no operation will be performed on dataframe, please specify drop=True ",UserWarning)
        ...
        if self.fill_missing is not None:
            ...
            if self.fill_missing not in ['mean','median']:
                raise ArgumentsError(f"Allowed argument is \"mean\" or \"median\" ")
    ...
    def execute(self):
        ...
        if self.drop is not None and self.fill_missing is None and self.fill_values is None and self.drop!=False:
            self.final_df = self.__drop_all_rows_with_null_values()

        elif self.fill_missing is not None and self.drop is None and self.fill_values is None:
            self.final_df = self.__fill_missing_with_mean_or_median()

        elif self.fill_values is not None and self.drop is None and self.fill_missing is None:
